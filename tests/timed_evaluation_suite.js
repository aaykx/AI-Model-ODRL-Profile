...
// ─── Timing instrumentation (added for Ch.8 per-layer cost measurement) ───────
// Not part of the original evaluation_suite.js; this file is a standalone
// copy so the validated original is never modified.
const queryTimings = []; // {ms, kind}
...
async function sparqlSelect(query) {
    ...
    queryTimings.push({ ms, kind: "select" });
...
(async () => {
    ...
    async function timed(name, fn) {
        ...
        sectionTimes[name] = Number(process.hrtime.bigint() - t0) / 1e6;
        return result;
    }

    const infraOk = await timed("A", sectionA);
    if (!infraOk) {
    ...
    await timed("V", sectionV);
    await timed("L", sectionL);
    await timed("G", sectionG);
    await timed("E", sectionE);
    await timed("X", sectionX);

    printSummary();

    console.log("\n" + "─".repeat(68));
    console.log("  PER-LAYER TIMING (added for Ch.8 cost analysis)");
    console.log("─".repeat(68));
    for (const [name, ms] of Object.entries(sectionTimes)) {
        console.log(`  Section ${name}: ${ms.toFixed(1)} ms`);
    }
    const totalQueryMs = queryTimings.reduce((s, t) => s + t.ms, 0);
    const selects = queryTimings.filter(t => t.kind === "select");
    const asks = queryTimings.filter(t => t.kind === "ask");
    console.log(`\n  Total SPARQL queries issued: ${queryTimings.length} (${selects.length} SELECT, ${asks.length} ASK)`);
    console.log(`  Total time inside SPARQL calls: ${totalQueryMs.toFixed(1)} ms`);
    console.log(`  Mean time per SPARQL call: ${(totalQueryMs / queryTimings.length).toFixed(2)} ms`);
    console.log(`  Min / Max single call: ${Math.min(...queryTimings.map(t => t.ms)).toFixed(2)} ms / ${Math.max(...queryTimings.map(t => t.ms)).toFixed(2)} ms`);
